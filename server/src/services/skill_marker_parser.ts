...
export class SkillMarkerParser {
   ...
   */
  private static readonly SKILL_ID_PATTERN = /^[a-z][a-z0-9_]*$/;
   ...
   * 支持嵌套 JSON（使用平衡括号匹配）
   */
  private static readonly SKILL_MARKER_REGEX = /skill:([a-z][a-z0-9_]*)\|(\{(?:[^{}]|\{[^{}]*\})*\})/g;
  ...
  parse(text: string): ParseResult {
    ...
    const errors: Array<{ position: number; message: string; rawText: string }> = [];
  ...
  async replaceAsync(
    text: string,
    replacer: (marker: SkillMarker) => Promise<string>
  ): Promise<string> {
    const result = this.parse(text);
