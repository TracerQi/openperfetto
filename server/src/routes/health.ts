...
export function setupHealthRoutes(server: FastifyInstance): void {
  ...
  server.get('/api/v1/health', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const response: HealthResponse = {
      status: 'ok',
      timestamp: now,
      traceId: (request as FastifyRequest & { traceId: string }).traceId || 'unknown',
      version: VERSION,
      uptime,
  ...
  server.get('/api/v1/health/live', async (_request: FastifyRequest, reply: FastifyReply) => {
    return reply.send({ status: 'ok' });
  });
  
  // 就绪探针（用于 k8s readiness probe）
  server.get('/api/v1/health/ready', async (_request: FastifyRequest, reply: FastifyReply) => {
