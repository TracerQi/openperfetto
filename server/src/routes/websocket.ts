...
import { ChatMessage } from '../types/session.js';
import { LLMProxy, LLMRequest, LLMStreamChunk } from '../services/llm_proxy.js';
...
let dependencies: WebSocketDependencies | null = null;

/**
 * 设置 WebSocket 路由依赖
 */
export function setWebSocketDependencies(deps: WebSocketDependencies): void {
  dependencies = deps;
...
class ConnectionPool {
  ...
  private readonly config = getConfig().websocket;
  ...
  closeConnection(connectionId: string, code: number, reason: string): void {
    ...
    try {
      ...
      logger.error('Error closing connection', err as Error, { connectionId });
    }
 ...
 * 处理 ping 消息
 */
function handlePing(info: ConnectionInfo, timestamp: number): void {
 ...
 */
async function handleChat(info: ConnectionInfo, message: ClientMessage & { type: 'chat' }): Promise<void> {
  ...
  try {
    ...
    loggerWithTrace.error('Chat handling failed', error as Error);
    sendMessage(info, {
      type: 'error',
      traceId,
      data: {
 ...
 */
function handleChatEchoMode(info: ConnectionInfo, message: ClientMessage & { type: 'chat' }): void {
  const { agentId, traceId, payload } = message;
 ...
 * 处理客户端消息
 */
async function handleMessage(info: ConnectionInfo, rawData: string): Promise<void> {
  ...
  switch (message.type) {
    ...
    case 'invoke_skill':
      ...
      break;
    default:
      loggerWithTrace.warn('Unknown message type', { type: (message as { type: string }).type });
      sendMessage(info, {
        type: 'error',
        traceId: info.traceId,
        data: {
 ...
 * 注册 WebSocket 路由
 */
export function setupWebSocketRoutes(server: FastifyInstance): void {
  server.get('/ws', { websocket: true }, (socket: WebSocket, req: FastifyRequest) => {
    ...
    try {
      ...
      loggerWithTrace.error('Failed to add connection', err as Error);
    ...
    socket.on('message', (rawData: Buffer) => {
      ...
      handleMessage(info, message).catch((err) => {
        logger.withTraceId(info.traceId).error('Message handling failed', err as Error);
      });
    });
