...
// 创建包含递归字段的完整 Schema
export const SkillStepSchema = BaseSkillStepSchema.extend({
...
export class YamlParser {
     ...
     * 解析 Skill YAML 内容
     */
    parseSkill(yamlContent, filePath) {
        const errors = [];
        const warnings = [];
        // 1. 解析 YAML
        let rawData;
        ...
        const processedData = this.preprocessYamlData(rawData);
        // 3. Zod 验证
    ...
    preprocessYamlData(data) {
        ...
        for (const [key, value] of Object.entries(data)) {
            ...
            if (value && typeof value === 'object' && !Array.isArray(value)) {
                result[newKey] = this.preprocessYamlData(value);
            }
            else if (Array.isArray(value)) {
                result[newKey] = value.map(item => {
                    if (item && typeof item === 'object') {
                        return this.preprocessYamlData(item);
                    }
                    return item;
                });
            }
...
//# sourceMappingURL=yaml_parser.js.map